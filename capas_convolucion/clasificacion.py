...
#---------------------------------------------
    def carga_tranf_imagen(self):
        ...
        if largo > alto:
            ...
            relacionAltura = int(round((20.0 / largo * self.altura), 0))
            if (relacionAltura == 0):
                relacionAltura = 1
        ...
        imagen_test = img.reshape(1, 28)

        prediccion = self.carga_modelo().predict_classes(imagen_test)
        print()
        print("La imagen es: "+clases[prediccion[0]])
        print()
        return imagen_test

    def extraccion_probabilidades(self):

        clases = ["Una camiseta/top","Un pantalón","Un jersey","Un vestido","Un abrigo","Una sandalia","Una camisa","Zapatillas","Un bolso","Botines"]
        #Extracción de las probabilidades
        probabilidades = self.carga_modelo().predict_proba(self.carga_tranf_imagen())
